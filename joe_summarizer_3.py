...
def summarize(text):
    ...
    textrank_scores = textrank.textrank(tokenized_sentences, True, '3-1-inf')

    feature_matrix = []
    # feature_matrix.append([0.2 * s for s in thematicity_feature_scores])
    # feature_matrix.append(sentence_position_scores)
    # feature_matrix.append(sentence_length_scores)
    # feature_matrix.append([0.2 * s for s in proper_noun_scores])
    ...
    # feature_matrix.append([1 * s for s in quotes_scores])
    # feature_matrix.append([0.2 * s for s in references_scores])
    feature_matrix.append([1 * s for s in textrank_scores])

    # features = ['  thema', 'sen_pos', 'sen_len', '  propn', '    num', ' tf_isf', 'cen_sim', '  upper', ' quotes',
    #             'referen', 'textran']
    # features = ['sen_len', 'numerals', 'upper_ca', 'textran']
    # features = ['themati', 'propnou', 'referen', 'textran']
    features = ['textran']
